...
skybox.set = function(player, number)
	if number == 0 then
		...
		player:get_meta():set_string("moontest_skybox:skybox", sky[1])
	end
end
...
skybox.clear = function(player)
	...
	player:get_meta():set_string("moontest_skybox:skybox", "off")
end
...
minetest.register_on_joinplayer(function(player)
	local sky = player:get_meta():get_string("moontest_skybox:skybox")
