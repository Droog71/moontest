...
local function preserve_metadata (pos, oldnode, oldmeta, drops)
	if #drops > 0 and drops[1]:get_name ():sub (1, 15) == "moontest_robots:robot" then
		local meta = minetest.get_meta (pos)
		local id = meta:get_int ("lwscratch_id")
...
local function after_place_node (pos, placer, itemstack, pointed_thing)
	...
	elseif not unique and placer and placer:is_player () then
		minetest.show_formspec (placer:get_player_name (),
										"moontest_robots:robot_set_owner",
			...
			if meta then
				...
				if inv then
					...
					if stack and not stack:is_empty () then
						...
						if stack:get_name () == "moontest_robots:cmd_line_insert" then
						...
						elseif stack:get_name () == "moontest_robots:cmd_line_remove" then
	...
	if not utils.can_interact_with_node (pos, clicker) then
		if clicker and clicker:is_player () then
			...
			minetest.show_formspec (clicker:get_player_name (),
											"moontest_robots:robot_privately_owned",
											spec)
		end
	end
...
minetest.register_node ("moontest_robots:robot", {
   description = "Robot\n" ..
   "Used to automate tasks outside the habitat.\n" ..
   "Perfect for research probes and REM extractors.",
...
minetest.register_node ("moontest_robots:robot_on", {
   description = "Robot\n" ..
   "Used to automate tasks outside the habitat.\n" ..
   "Perfect for research probes and REM extractors.",
   tiles = { "lw_itch_top.png", "lw_itch_bottom.png", "lw_itch_left.png",
...
minetest.register_on_player_receive_fields (function (player, formname, fields)
   if formname == "moontest_robots:robot_stop" and
...
minetest.register_on_player_receive_fields (function (player, formname, fields)
   if formname == "moontest_robots:robot_set_owner" and
		player and player:is_player () then
