...
dofile(minetest.get_modpath("portable_power") .. DIR_DELIM .. "nodes.lua")
dofile(minetest.get_modpath("portable_power") .. DIR_DELIM .. "abm.lua")
...
function clear_power(pos)
    for z = pos.z + 1, pos.z + 20, 1 do
        ...
        if minetest.get_node(hit).name == "portable_power:power_z" then
    ...
    for x = pos.x + 1, pos.x + 20, 1 do
        ...
        if minetest.get_node(hit).name == "portable_power:power_x" then
            minetest.remove_node(hit)
        end
    end
    for x = pos.x - 1, pos.x - 20, -1 do
    ...
    for y = pos.y + 1, pos.y + 20, 1 do
        ...
        if minetest.get_node(hit).name == "portable_power:power_y" then
            minetest.remove_node(hit)
        end
    end
    for y = pos.y - 1, pos.y - 20, -1 do
...
function transmitter_blocked(hit)
    local clear_nodes = {
        "air",
        "ignore",
        "portable_power:power_x",
        "portable_power:power_y",
        "portable_power:power_z"
    }
